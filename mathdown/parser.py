...
div = pp.Forward()
equation = pp.Combine(pp.QuotedString('$$', multiline=True)('body').set_parse_action(Equation()) + pp.Optional(single_newline + '其中' + div))
...
table.set_parse_action(table_to_latex)

paragraph = code | center | table | pp.DelimitedList(ulist | olist | div, single_newline)
...
def_key = pp.Suppress('**定义') + pp.Optional(pp.Suppress(pp.nums)) + pp.Optional((pp.QuotedString('[', end_quote_char=']') | pp.QuotedString('（', end_quote_char='）') | title).set_parse_action(lambda t: t[0])('name')) + pp.Suppress('**')
...
hrule = pp.Literal('---').set_parse_action(lambda t: r'\hrule')
chapter = (zero_or_more_newlines + chapter_title + pp.Optional(one_or_more_newlines + text) + one_or_more_newlines + pp.DelimitedList(section, one_or_more_newlines)
