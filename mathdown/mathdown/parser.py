...
div = pp.Forward()
equation = pp.Combine((pp.QuotedString('$$', multiline=True)|pp.QuotedString('\\[', end_quote_char='\\]', multiline=True))('body').set_parse_action(Equation()) + pp.Optional(single_newline + '其中' + div))
...
center = pp.QuotedString(*pp.make_html_tags('center'), multiline=True)('body')  #.set_parse_action(LatexEnv('center'))
...
paragraph = code | table_with_caption | center | figure | pp.DelimitedList(ulist | olist | div, single_newline)
