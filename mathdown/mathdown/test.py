...
# print(algorithm.transform_string(r"""**算法 ICA生成算法**
# 输入：数据集$X$
# 返回：新的数据集$X^*$
...
print(div.parse_string("""$C$是一个只与$X$有关的常数。此时，归结为下述优化问题——这是一个用交叉熵作为损失函数的优化问题：
$$
\min \{L(W,H)=H(X,WH)\}.
$$
...
print(chapter.parse_string(r"""# 降维
...
$$
其中$\phi$和$\psi$分别为编码器和解码器，$l(\cdot)$是某个损失函数。特别当$\psi,\phi$是线性算子时，称之为**线性自编码器**。显然截断SVD的自编码器形式给出了在线性条件下的一个解。因此，截断SVD是一种线性自编码器。
